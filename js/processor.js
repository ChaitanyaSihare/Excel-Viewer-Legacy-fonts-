...
// ---- Legacy font detection helpers ----
    ...
    function detectFontNameInFile(rawBytes, workbook, fileName, biffFontMap) {
      ...
      let detectedNames = [];
      let standardNamesFound = []; // normal/Unicode-safe fonts actually seen — previously silently dropped entirely
      ...
      if (stylesFile && stylesFile.content) {
        try {
          ...
          const usedNames = [...new Set([...usedFontIds].map(id => fontIdToName[id]).filter(Boolean))];
          detectedNames = usedNames.filter(n => !STANDARD_DEFAULT_FONTS.has(n.toLowerCase()));
          standardNamesFound = usedNames.filter(n => STANDARD_DEFAULT_FONTS.has(n.toLowerCase()));
      ...
      if (!detectedNames.length && !standardNamesFound.length && biffFontMap) {
        const names = new Set();
        const standardNames = new Set();
        Object.keys(biffFontMap).forEach(sheetName => {
          Object.values(biffFontMap[sheetName]).forEach(name => {
            if (!name) return;
            if (STANDARD_DEFAULT_FONTS.has(name.toLowerCase())) standardNames.add(name);
            else names.add(name);
          });
        });
        detectedNames = [...names];
        standardNamesFound = [...standardNames];
      }

      if (!detectedNames.length && !standardNamesFound.length) {
      ...
      } else if (standardNamesFound.length) {
        // Previously this whole branch just hid the box — meaning a file
        // using only normal fonts (Arial, Calibri, Mangal...) gave zero
        // feedback at all. Report what was actually found instead of
        // saying nothing.
        resultBox.style.display = 'block';
        resultBox.className = 'status success';
        resultBox.textContent = t('fileUsesStandardFont', { fonts: standardNamesFound.join(', ') });
      } else {
...
// ---- Address & per-cell font metadata (the ground-truth approach) ----
    ...
    function isLegacyFontName(name) {
      ...
      return lower.includes('kruti') || lower.includes('devlys') ||
        ...
        lower.includes('amar ujala');
    }


// ---- Legacy-vs-Unicode decision for export ----
    function cellNeedsConversion(cellFonts, address, cellText) {
...
// ---- Unicode Excel export ----
    // Converts ONE sheet's data to unicode, returning the new worksheet
    // plus a tally of any cells left unconverted for lack of a registered
    // converter (see the comment on pickConverterForCell). Pulled out of
    // exportToUnicodeExcel so the same logic — previously tied to
    // "whichever sheet is currently on screen" — can run for every sheet
    // in the workbook when exporting the whole file, not just the one
    // being viewed.
    function convertSheetToUnicode(workbook, sheetName, forceEnglish) {
      const worksheet = workbook.Sheets[sheetName];
      const jsonData = XLSX.utils.sheet_to_json(worksheet, { header: 1, raw: false });
      const cellFonts = getSheetCellFonts(workbook, sheetName);
      ...
      const suspiciousMismatchAddresses = [];
      const converted = jsonData.map((row, r) => row.map((cell, c) => {
        const text = cell != null ? String(cell) : '';
        if (!text) return text;
        // The user explicitly marked this sheet as already-correct English
        // via the per-sheet toggle in the viewer — exporting the whole
        // workbook must respect that the same way viewing it does, or a
        // sheet the user deliberately opted out of conversion for would
        // get silently re-converted (and likely corrupted) here anyway.
        if (forceEnglish) return text;
        const address = colIndexToLetters(c) + (r + 1);
        const knownFont = cellFonts ? cellFonts[address] : null;
        if (cellNeedsConversion(cellFonts, address, text)) {
          const converter = pickConverterForCell(knownFont);
          if (!converter) {
            const label = knownFont || t('exportSkippedUnknownFont');
            skippedByFont[label] = (skippedByFont[label] || 0) + 1;
            return text; // recognized as legacy but no converter built for this font yet — leave as-is rather than mis-convert
          }
          try { return converter(text, isProtectedWord); } catch (err) { return text; }
        }
        // Font metadata says this cell is fine — normally authoritative,
        // but neither font-reading path here parses per-character rich-
        // text runs within a cell, so a cell that mixes a legacy-font
        // clause with a differently-styled date/number run can report the
        // WRONG single font for the whole cell. This flags that mismatch
        // for the user rather than guessing: converting the whole cell
        // through Kruti Dev's mapping would also remap the genuinely fine
        // ASCII parts (a comma or "Dt" gets its own glyph in that font
        // too), corrupting content that was never broken. See
        // looksLikeUnconvertedLegacyGlyphs in converter.js.
        if (knownFont && !isLegacyFontName(knownFont) && looksLikeUnconvertedLegacyGlyphs(text)) {
          suspiciousMismatchCount++;
          if (suspiciousMismatchAddresses.length < 5) suspiciousMismatchAddresses.push(address);
        }
        return text;
      }));
      const newWs = XLSX.utils.aoa_to_sheet(converted);

      // Carry formulas over too — aoa_to_sheet only wrote plain values,
      // so a formula cell would otherwise silently freeze into a static
      // number on export. The cached value SheetJS just wrote stays as
      // the displayed value; adding .f back on top means Excel will
      // still recalculate it live if the user edits an input cell.
      // Formula TEXT itself (like "SUM") is never Kruti Dev-encoded —
      // only cell CONTENT is — so this needs no conversion, just a copy.
      Object.keys(worksheet).forEach(addr => {
        if (addr.charAt(0) === '!') return;
        const srcCell = worksheet[addr];
        if (srcCell && typeof srcCell.f === 'string' && srcCell.f && newWs[addr]) {
          newWs[addr].f = srcCell.f;
        }
      });

      return { newWs, skippedByFont, suspiciousMismatchCount, suspiciousMismatchAddresses };
    }

    // Excel sheet names must be <=31 chars and unique within a workbook.
    // Names copied straight from the source workbook already satisfy
    // both (the source file couldn't exist otherwise) — but truncating a
    // long name to 31 chars is new here (only ever exported one sheet
    // before, so no truncation collision was possible), and two long
    // names that only differ after character 31 could collide. Cheap
    // safety net rather than a real expectation of hitting it often.
    function safeSheetName(name, usedNames) {
      const base = (name || 'Sheet1').slice(0, 31);
      let candidate = base;
      let n = 1;
    ...
    async function exportToUnicodeExcel() {
      ...
      try {
        ...
        const skippedByFontAcrossSheets = {};
        let mismatchCountAcrossSheets = 0;
        const mismatchAddressesAcrossSheets = [];
        currentWorkbook.SheetNames.forEach(sheetName => {
          ...
          XLSX.utils.book_append_sheet(newWb, newWs, safeSheetName(sheetName, usedNames));
          Object.keys(skippedByFont).forEach(font => {
            skippedByFontAcrossSheets[font] = (skippedByFontAcrossSheets[font] || 0) + skippedByFont[font];
          });
          mismatchCountAcrossSheets += suspiciousMismatchCount;
          suspiciousMismatchAddresses.forEach(addr => {
            if (mismatchAddressesAcrossSheets.length < 5) mismatchAddressesAcrossSheets.push(sheetName + '!' + addr);
          });
        });
        const wbBuffer = XLSX.write(newWb, { type: 'array', bookType: 'xlsx' });
        const wbBlob = new Blob([wbBuffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
        await saveBlobCrossPlatform(wbBlob, 'yatharoop-unicode.xlsx');

        const messages = [];
        const skippedFonts = Object.keys(skippedByFontAcrossSheets);
        if (skippedFonts.length) {
          const details = skippedFonts
            .map(font => {
              const count = skippedByFontAcrossSheets[font];
              return t('exportSkippedDetail', { font, count, plural: count === 1 ? '' : 's' });
            })
            .join(t('exportSkippedJoiner'));
          messages.push(t('exportSkippedSummary', { details }));
        }
        if (mismatchCountAcrossSheets > 0) {
          messages.push(t('exportMismatchSummary', {
            count: mismatchCountAcrossSheets,
            plural: mismatchCountAcrossSheets === 1 ? '' : 's',
            addresses: mismatchAddressesAcrossSheets.join(', ') + (mismatchCountAcrossSheets > mismatchAddressesAcrossSheets.length ? '…' : '')
          }));
        }
        if (messages.length) {
          setStatus(messages.join(' '), 'warning');
        } else {
          setStatus(null); // clear any stale warning/error from a previous export attempt
        }
      } catch (err) {
        console.error(err);
