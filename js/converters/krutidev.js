// converters/krutidev.js
// Kruti Dev -> Unicode Hindi character-mapping converter.
...
// Registers itself with converter.js's registry — adding DevLys/Chanakya/
// etc. later means a new file exactly like this one, registering under a
// different font-name pattern. Nothing in processor.js or ui.js needs to
// change to add a new legacy font converter.
// Depends on: converter.js (replaceAllLiteral, registerLegacyConverter).
...
function krutiDevToUnicode(input, isProtectedWord) {
  ...
  // Also covers comma-separated lists of these ("2, 9, 39, 48a, 49a, 49b" —
  // real example: section numbers in a legal clause) as ONE protected span,
  // not just each number individually — a comma in a real Devanagari
  // sentence maps to a genuine glyph (ए), so a bare list-separator comma
  // was still getting mangled even with each number around it protected.
  const protectedTokens = [];
  text = text.replace(/\b\d[\d./]*[a-zA-Z]{0,2}(?:\s*,\s*\d[\d./]*[a-zA-Z]{0,2})*\b/g, (match) => {
...
registerLegacyConverter('kruti', krutiDevToUnicode);
