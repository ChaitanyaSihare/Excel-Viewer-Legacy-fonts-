...
// ---- Element references ----
    ...
    const docViewModeUnicodeBtn = document.getElementById('docViewModeUnicodeBtn');
    const docViewModeLegacyBtn = document.getElementById('docViewModeLegacyBtn');
    const docViewModeWarning = document.getElementById('docViewModeWarning');
    ...
    let activeFontBuffer = null; // raw bytes of the user's uploaded font, kept for PDF embedding — never redistributed, only used for THIS export in THIS browser session
...
// ---- Bilingual UI (Hindi / English) ----
    const TRANSLATIONS = {
      hi: {
        ...
        infoPopupBtnLabel: 'ⓘ जानकारी',
        infoPopupTitle: 'यह टूल कैसे काम करता है',
        ...
        trustBadge: 'आपकी फाइल आपके डिवाइस में ही रहती है — सर्वर पर अपलोड नहीं होती',
        exportUnicodeDesc: 'यूनिकोड Excel में पुराना फॉन्ट टेक्स्ट यूनिकोड में बदल दिया जाता है — कुछ अस्पष्ट शब्दों की जाँच ज़रूरी हो सकती है, हर सेल पर टैप करके ठीक किया जा सकता है।',
        compatNote: 'यह टूल मुख्य रूप से सेल का टेक्स्ट और तालिका दिखाने के लिए है — जटिल formulas, charts, macros या conditional formatting हूबहू नहीं दिख सकते।',
        ...
        docxPanelTitle: 'Word फाइल को यूनिकोड में बदलें',
        docxPanelSubtitle: 'केवल .docx (पुराना .doc अभी समर्थित नहीं है)',
        docxChooseLabel: 'Word फाइल चुनें',
        docxConvertLabel: 'बदलें और डाउनलोड करें',
        docxConverting: 'फाइल बदली जा रही है...',
        docxDone: '{count} जगह बदली गईं। फाइल डाउनलोड हो गई।',
        docxNoLegacyFont: 'इस फाइल में कोई पुराना फॉन्ट नहीं मिला — बदलने की ज़रूरत नहीं।',
        docxSkippedFonts: ' {font} के लिए कोई converter उपलब्ध नहीं — वह हिस्सा नहीं बदला गया।',
        docxError: 'यह फाइल नहीं बदली जा सकी। पक्का करें यह एक असली .docx फाइल है।',
        docxNotDocFile: '.doc (पुराना फॉर्मेट) अभी समर्थित नहीं है — केवल .docx चलेगा।',
        ...
        pdfPickerTitle: 'PDF में कौन सी शीट शामिल करें?',
        pdfPickerConfirm: 'PDF बनाएं',
        pdfFontChoiceTitle: 'PDF किस फॉन्ट में बने?',
        pdfFontChoiceUnicodeDesc: 'यूनिकोड अनुमान — हमेशा उपलब्ध, असली टेक्स्ट, पर अनुमान है',
        pdfFontChoiceOriginalDesc: 'आपका अपलोड किया फॉन्ट — बिल्कुल असली दिखावट, पर आपका अनुमान लगाना ज़रूरी नहीं',
        cancelLabel: 'रद्द करें',
        pdfPickerNoneSelected: 'कम से कम एक शीट चुनें।',
      ...
      en: {
        ...
        infoPopupBtnLabel: 'ⓘ Info',
        infoPopupTitle: 'How this tool works',
        ...
        trustBadge: 'Your file stays on your device — never uploaded to a server',
        exportUnicodeDesc: 'The Unicode Excel converts legacy-font text to Unicode — some unclear words may need checking, tap any cell to correct it.',
        compatNote: 'This tool is mainly for viewing cell text and tables — complex formulas, charts, macros, or conditional formatting may not appear exactly as in Microsoft Excel.',
        ...
        docxPanelTitle: 'Convert Word file to Unicode',
        docxPanelSubtitle: 'Only .docx (old .doc not supported yet)',
        docxChooseLabel: 'Choose Word file',
        docxConvertLabel: 'Convert and download',
        docxConverting: 'Converting file...',
        docxDone: '{count} spots converted. File downloaded.',
        docxNoLegacyFont: 'No legacy font found in this file — no conversion needed.',
        docxSkippedFonts: ' No converter available for {font} — that part was left unconverted.',
        docxError: 'Could not convert this file. Make sure it is a real .docx file.',
        docxNotDocFile: '.doc (old format) is not supported yet — only .docx works.',
        ...
        pdfPickerTitle: 'Which sheets should the PDF include?',
        pdfPickerConfirm: 'Create PDF',
        pdfFontChoiceTitle: 'Which font should the PDF use?',
        pdfFontChoiceUnicodeDesc: 'Unicode guess — always available, real text, but a guess',
        pdfFontChoiceOriginalDesc: 'Your uploaded font — exact original look, no guessing needed',
        cancelLabel: 'Cancel',
        pdfPickerNoneSelected: 'Select at least one sheet.',
...
// protection entirely, which was the source of the bad-looking output.
    function renderCellHTML(cellText, trustedLegacy) {
      ...
      return runs.map(run => {
        ...
        // Pull out digit-anchored case-number/date/list segments FIRST, as
        // whole protected spans — mirrors the exact same protection
        // converter.js's krutiDevToUnicode does for export. Catches
        // "35635/07", "48a", and comma-separated lists like "2, 9, 39,
        // 48a, 49a, 49b" as single units, so a legal clause's section list
        // doesn't get its connecting commas rendered as Devanagari glyphs
        // (real Kruti Dev maps ',' to a genuine character) while only the
        // numbers around them were protected individually.
        const NUMERIC_SPAN = /\b\d[\d./]*[a-zA-Z]{0,2}(?:\s*,\s*\d[\d./]*[a-zA-Z]{0,2})*\b/g;
        const segments = run.text.split(NUMERIC_SPAN);
        const numericMatches = run.text.match(NUMERIC_SPAN) || [];
        let html = '';
        segments.forEach((segment, segIdx) => {
          html += renderNonNumericPieces(segment, trustedLegacy);
          if (segIdx < numericMatches.length) {
            html += '<span class="run-normal-font">' + escapeHTML(numericMatches[segIdx]) + '</span>';
          }
        });
        return html;
      }).join('');
    ...
    // The remainder of a run after digit-anchored numeric spans have
    // already been pulled out above — handles plain words (dictionary/
    // exceptions check) and lone punctuation still touching a digit on
    // exactly one side (e.g. a trailing '.' right after a protected span).
    function renderNonNumericPieces(text, trustedLegacy) {
        const pieces = text.match(/\d+|[A-Za-z]+|[^\dA-Za-z]/g) || [];
...
// ---- Sheet rendering ----
    ...
    let currentDocxParagraphs = null;
    ...
    function setViewMode(mode) {
      ...
      if (currentDocxParagraphs) renderDocumentView();
    }
...
// ---- File loading ----
    async function processDocxFile(bytes, fileName) {
      tableWrapper.style.display = 'none';
      documentWrapper.style.display = 'none';
      emptyState.style.display = 'none';
      const summaryEl = document.getElementById('workbookSummary');
      if (summaryEl) summaryEl.style.display = 'none';
      tableInfo.textContent = 'फाइल पढ़ी जा रही है...';
      try {
        const paragraphs = await extractDocxParagraphs(bytes);
        currentDocxBytes = bytes;
        currentDocxFileName = fileName;
        currentDocxParagraphs = paragraphs;
        currentWorkbook = null;
        currentSheetName = null;
        renderDocumentView();
        tableInfo.textContent = 'फाइल सफलतापूर्वक पढ़ी गई।';
        setTimeout(() => documentWrapper.scrollIntoView({ behavior: 'smooth', block: 'start' }), 120);
      } catch (err) {
        console.error(err);
        setStatus('यह Word फाइल नहीं खुल पाई। पक्का करें यह एक असली .docx फाइल है।', 'error');
        emptyState.style.display = 'block';
      }
    }

    function renderDocumentView() {
      if (!currentDocxParagraphs) return;
      docViewModeUnicodeBtn.classList.toggle('btn-primary', viewMode === 'unicode');
      docViewModeLegacyBtn.classList.toggle('btn-primary', viewMode === 'legacy');
      docViewModeWarning.style.display = viewMode === 'unicode' ? 'block' : 'none';
      let html = '';
      currentDocxParagraphs.forEach(runs => {
        let paraHtml = '';
        runs.forEach(run => {
          if (viewMode === 'unicode' && run.isLegacy) {
            const converter = (typeof getConverterForFont === 'function' && getConverterForFont(run.fontName)) ||
              (typeof krutiDevToUnicode === 'function' ? krutiDevToUnicode : null);
            paraHtml += escapeHTML(converter ? converter(run.text, viewerIsProtectedWord) : run.text);
          } else {
            paraHtml += escapeHTML(run.text);
          }
        });
        html += '<p style="margin:0 0 10px;">' + (paraHtml || '&nbsp;') + '</p>';
      });
      documentContent.innerHTML = html;
      documentMeta.textContent = currentDocxFileName + ' • ' + currentDocxParagraphs.length + ' पैराग्राफ़';
      documentWrapper.style.display = 'block';
    }

    if (docViewModeUnicodeBtn) {
      docViewModeUnicodeBtn.addEventListener('click', () => setViewMode('unicode'));
      docViewModeLegacyBtn.addEventListener('click', () => setViewMode('legacy'));
    }

    const docNewFileBtn = document.getElementById('docNewFileBtn');
    if (docNewFileBtn) {
      docNewFileBtn.addEventListener('click', () => {
        documentWrapper.style.display = 'none';
        emptyState.style.display = 'block';
        currentDocxBytes = null;
        currentDocxFileName = null;
        currentDocxParagraphs = null;
        fileInput.value = '';
    ...
    const docExportDocxBtn = document.getElementById('docExportDocxBtn');
    if (docExportDocxBtn) {
      docExportDocxBtn.addEventListener('click', async () => {
        if (!currentDocxBytes) return;
        docExportDocxBtn.disabled = true;
        try {
          const result = await convertDocxToUnicode(currentDocxBytes, viewerIsProtectedWord);
          const outName = currentDocxFileName.replace(/\.docx$/i, '') + '-unicode.docx';
          await saveBlobCrossPlatform(result.blob, outName);
          setStatus(t('docxDone', { count: result.convertedCount }), 'success');
        } catch (err) {
          console.error(err);
          setStatus(t('docxError'), 'error');
        } finally {
          docExportDocxBtn.disabled = false;
        }
      });
    }

    const docExportPdfBtn = document.getElementById('docExportPdfBtn');
    if (docExportPdfBtn) {
      docExportPdfBtn.addEventListener('click', async () => {
        if (!currentDocxParagraphs) return;
        docExportPdfBtn.disabled = true;
        try {
          const { jsPDF } = window.jspdf;
          const pdf = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' });
          ensureDevanagariFontRegistered(pdf);
          pdf.setFont('NotoDeva');
          pdf.setFontSize(12);
          const marginLeft = 15, marginTop = 20, pageWidth = 180, pageBottom = 280;
          let y = marginTop;
          currentDocxParagraphs.forEach(runs => {
            let text = '';
            runs.forEach(run => {
              if (viewMode === 'unicode' && run.isLegacy) {
                const converter = (typeof getConverterForFont === 'function' && getConverterForFont(run.fontName)) ||
                  (typeof krutiDevToUnicode === 'function' ? krutiDevToUnicode : null);
                text += converter ? converter(run.text, viewerIsProtectedWord) : run.text;
              } else {
                text += run.text;
              }
            });
            ...
            lines.forEach(line => {
              if (y > pageBottom) { pdf.addPage(); y = marginTop; }
              pdf.text(line, marginLeft, y);
              y += 7;
            });
            y += 3;
          });
          await saveBlobCrossPlatform(pdf.output('blob'), (currentDocxFileName || 'document').replace(/\.docx$/i, '') + '.pdf');
        } catch (err) {
          console.error(err);
          setStatus('PDF नहीं बन पाया।', 'error');
        } finally {
          docExportPdfBtn.disabled = false;
        }
      });
    }
    ...
    function readExcel() {
      ...
      reader.onload = function(e) {
        ...
        if (/\.docx$/i.test(file.name)) {
          processDocxFile(bytes, file.name);
        } else {
          processWorkbookData(bytes, file.name);
        }
      };
...
// ---- Sheet switcher, download-as-HTML, misc event wiring ----
    ...
    async function downloadCurrentHTML() {
      ...
      await saveBlobCrossPlatform(blob, 'yatharoop.html');
    }
    ...
    if ('launchQueue' in window && window.launchQueue) {
      window.launchQueue.setConsumer(async (launchParams) => {
        ...
        try {
          ...
          const isFontFile = /\.(ttf|otf)$/i.test(file.name);
          if (isFontFile) {
            // Same pipeline as manually picking a font via fontFileInput —
            // reused directly rather than duplicated, so anything that
            // changes there (persistence, PDF embedding, etc.) stays in
            // sync automatically.
            const buffer = await file.arrayBuffer();
            const ok = await applyLegacyFontBuffer(buffer, file.name);
            if (ok) {
              fontNameBox.style.display = 'block';
              fontNameBox.innerHTML = '<strong>चयनित फॉन्ट:</strong> ' + escapeHTML(file.name);
              if (typeof saveLastFont === 'function') saveLastFont(file.name, buffer);
            }
          } else {
    ...
    // ---- Capacitor Android: "Open with" / "Share to" यथारूप ----
    // The File Handling API above is a WEB platform feature and does NOT
    // fire inside a Capacitor app — Capacitor wraps this site in a real
    // native WebView, not a Trusted Web Activity, so Android routes
    // "Open with"/"Share to" intents to native code instead, and nothing
    // reaches the page unless a Capacitor plugin explicitly forwards it.
    //
    // This listens for @capgo/capacitor-share-target's event IF that
    // plugin is installed and registered natively — entirely inert
    // otherwise (checked defensively at every level below), so it's safe
    // to ship even before the native side is finished. Getting the actual
    // file bytes off a content:// URI needs Capacitor's own Filesystem
    // plugin (not a browser File object), so this branch is intentionally
    // separate from the File Handling code above rather than unified with it.
    //
    // NOT independently verified against a real device/build — I have no
    // access to the native Android project this depends on (a separate
    // repo from this web app), so the exact plugin/event name here may
    // need adjusting to match whatever actually gets installed. Whoever
    // wires up the native side should sanity-check this block against
    // that plugin's actual current API before relying on it.
    try {
      const cap = window.Capacitor;
      const shareTarget = cap && cap.Plugins && cap.Plugins.ShareTarget;
      if (shareTarget && typeof shareTarget.addListener === 'function') {
        shareTarget.addListener('shareReceived', async (data) => {
          const sharedFile = data && data.files && data.files[0];
          if (!sharedFile || !sharedFile.uri) return;
          try {
            const filesystem = cap.Plugins.Filesystem;
            if (!filesystem) {
              console.warn('Capacitor Filesystem plugin not available — cannot read shared file content.');
              return;
            }
            const isFontFile = /\.(ttf|otf)$/i.test(sharedFile.name || '');
            const result = await filesystem.readFile({ path: sharedFile.uri });
            // Capacitor's Filesystem.readFile returns base64 by default.
            const binary = atob(result.data);
            const bytes = new Uint8Array(binary.length);
            for (let i = 0; i < binary.length; i++) bytes[i] = binary.charCodeAt(i);
            if (isFontFile) {
              const ok = await applyLegacyFontBuffer(bytes.buffer, sharedFile.name || 'font.ttf');
              if (ok) {
                fontNameBox.style.display = 'block';
                fontNameBox.innerHTML = '<strong>चयनित फॉन्ट:</strong> ' + escapeHTML(sharedFile.name || '');
                if (typeof saveLastFont === 'function') saveLastFont(sharedFile.name, bytes.buffer);
              }
            } else {
              if (!isValidExcelSignature(bytes)) {
                setStatus('साझा की गई फाइल पहचानी नहीं जा सकी।', 'error');
                return;
              }
              processWorkbookData(bytes, sharedFile.name || 'shared.xlsx');
            }
          } catch (err) {
            console.error('Could not read file shared via Capacitor:', err);
            setStatus('साझा की गई फाइल नहीं खुल पाई।', 'error');
          }
        });
      }
    } catch (err) {
      // window.Capacitor not present at all (normal browser/PWA use) — expected, not an error.
    }
...
// ---- PDF export ----
    function arrayBufferToBase64(buffer) {
      ...
      const chunkSize = 0x8000; // avoid call-stack issues on very large fonts
    ...
    function ensureDevanagariFontRegistered(pdf) {
      // Embeds Noto Sans Devanagari (SIL Open Font License — safe to bundle,
      // unlike Kruti Dev/DevLys/Chanakya) as a REAL PDF font. This is what
      // makes PDF text real, selectable, searchable text instead of a
      // rasterized picture — and is why pagination can be computed
      // mathematically from actual font metrics instead of guessed from a
      // DOM screenshot. Registered once per jsPDF document instance.
      pdf.addFileToVFS('NotoSansDevanagari.ttf', NOTO_DEVANAGARI_BASE64);
      pdf.addFont('NotoSansDevanagari.ttf', 'NotoDeva', 'normal');
      pdf.addFont('NotoSansDevanagari.ttf', 'NotoDeva', 'bold'); // same file for both — avoids autoTable's "no bold variant" warning on header rows

      // If the user has already uploaded their own legacy font (for the
      // on-screen viewer), embed THAT one too — same file, same "never
      // redistributed, only used in this session" trust boundary already
      // established for viewing. This lets PDF export show text exactly
      // as the user's own font renders it, without depending on our
      // conversion being right — useful when they don't fully trust the
      // Unicode guess and want the PDF to visually match what they see in
      // "मूल फॉन्ट" view.
      let userFontRegistered = false;
      if (activeFontBuffer) {
        try {
          const b64 = arrayBufferToBase64(activeFontBuffer);
          pdf.addFileToVFS('UserLegacyFont.ttf', b64);
          pdf.addFont('UserLegacyFont.ttf', 'UserLegacy', 'normal');
          pdf.addFont('UserLegacyFont.ttf', 'UserLegacy', 'bold');
          userFontRegistered = true;
        } catch (err) {
          console.error('Could not embed user font into PDF, falling back to Unicode guess:', err);
        }
      }
      return userFontRegistered;
    }

    // Builds an autoTable-ready body: a 2D array where a merged cell's
    // top-left becomes { content, colSpan, rowSpan } and every other cell
    // covered by that merge is simply omitted from its row (autoTable's
    // own convention — it is NOT the same shape as the on-screen
    // spanInfo grid used by ui.js's renderSheet, which keeps a placeholder
    // for every covered cell instead).
    function buildAutoTableBody(worksheet, textGrid) {
      const merges = worksheet['!merges'] || [];
      const covered = new Set(); // "r,c" strings for non-origin cells inside a merge
      const originInfo = {}; // "r,c" -> { rowSpan, colSpan }
      merges.forEach(m => {
        originInfo[m.s.r + ',' + m.s.c] = { rowSpan: m.e.r - m.s.r + 1, colSpan: m.e.c - m.s.c + 1 };
      ...
      const maxCols = textGrid.reduce((max, row) => Math.max(max, row.length), 0);
      return textGrid.map((row, r) => {
        const outRow = [];
        for (let c = 0; c < maxCols; c++) {
          if (covered.has(r + ',' + c)) continue;
          const text = row[c] != null ? String(row[c]) : '';
          const span = originInfo[r + ',' + c];
          outRow.push(span ? { content: text, rowSpan: span.rowSpan, colSpan: span.colSpan } : text);
        }
        return outRow;
      });
    }

    async function saveBlobCrossPlatform(blob, filename) {
      // pdf.save()/a normal <a download> click relies on the BROWSER's
      // download manager to catch a blob-URL navigation. A Capacitor app
      // is a native WebView, not a full browser — there's no download UI
      // wired up unless the native side explicitly handles it, so this
      // can silently do nothing at all (no error, no file, nothing).
      // Fix: when running inside Capacitor, write the file directly via
      // its own native Filesystem plugin instead of trying to trigger a
      // browser download that the WebView may not know how to catch.
      const cap = window.Capacitor;
      ...
      if (filesystem && typeof filesystem.writeFile === 'function') {
        try {
          ...
          // 'Documents' matches Capacitor's own documented default for
          // user-facing saved files — NOT independently verified against
          // a real build, since I have no device/APK to test against;
          // double check this actually lands somewhere the user can find
          // it (a file manager, a "Share" flow, etc.) before relying on it.
          await filesystem.writeFile({
            path: filename,
            data: base64,
            directory: 'DOCUMENTS',
          });
          setStatus('फाइल सहेजी गई: ' + filename, 'success');
          return true;
        } catch (err) {
          console.error('Capacitor Filesystem save failed, falling back to browser download:', err);
          // fall through to the standard browser path below
        }
      }
    ...
    async function exportToPdf(sheetNamesToInclude, useOriginalFont) {
      if (!currentWorkbook || !currentSheetName) {
        setStatus('पहले कोई फाइल खोलें।', 'error');
        return;
      }
      const sheetsForPdf = (sheetNamesToInclude && sheetNamesToInclude.length)
        ? currentWorkbook.SheetNames.filter(name => sheetNamesToInclude.includes(name))
      ...
      try {
        ...
        const userFontAvailable = ensureDevanagariFontRegistered(pdf);
        // Explicit choice from the sheet-picker UI now, not inferred from
        // whatever view mode happened to be active — someone could easily
        // be in "मूल फॉन्ट" view for reading but still want the safer,
        // always-correct Unicode PDF, or vice versa.
        const useUserFont = !!useOriginalFont && userFontAvailable;
        const pdfFontFamily = useUserFont ? 'UserLegacy' : 'NotoDeva';
        pdf.setFont(pdfFontFamily);

        const allSkipped = {};
        let mismatchCountTotal = 0;
        const mismatchAddresses = [];
        let isFirstSheet = true;

        for (const sheetName of sheetsForPdf) {
          const worksheet = currentWorkbook.Sheets[sheetName];
          const jsonData = XLSX.utils.sheet_to_json(worksheet, { header: 1, raw: false });
          if (!jsonData.length) continue; // an empty sheet contributes no pages, same as the on-screen empty-state

          let textGrid;
          if (useUserFont) {
            // Raw text, exactly as stored in the file — no conversion
            // attempted at all. Correct by construction: it's literally
            // what the user's own font already displays on screen.
            textGrid = jsonData;
          } else {
            const forceEnglish = englishSheets.has(sheetName);
            const { newWs, skippedByFont, suspiciousMismatchCount, suspiciousMismatchAddresses } =
              convertSheetToUnicode(currentWorkbook, sheetName, forceEnglish);
            Object.keys(skippedByFont).forEach(font => {
              allSkipped[font] = (allSkipped[font] || 0) + skippedByFont[font];
            });
            mismatchCountTotal += suspiciousMismatchCount;
            suspiciousMismatchAddresses.forEach(addr => {
              if (mismatchAddresses.length < 5) mismatchAddresses.push(sheetName + '!' + addr);
            });
            textGrid = XLSX.utils.sheet_to_json(newWs, { header: 1, raw: false });
          }
          const body = buildAutoTableBody(worksheet, textGrid);
          ...
          pdf.autoTable({
            ...
            styles: {
              ...
              fontSize: 8.5,
              cellPadding: 1.5,
              overflow: 'linebreak', // wraps at real word boundaries using actual font metrics — not CSS approximation, and never breaks mid-word unless one word alone exceeds the column
              valign: 'middle',
            },
            theme: 'grid',
          });
        ...
        if (isFirstSheet) {
          // Every selected sheet was empty — nothing was ever added beyond
          // the initial blank document jsPDF starts with.
          setStatus('चयनित शीट्स में कोई डेटा नहीं मिला।', 'error');
        } else {
          await saveBlobCrossPlatform(pdf.output('blob'), 'yatharoop.pdf');
          // Same transparency the Excel exporter already gives — reusing
          // its exact messages/keys rather than inventing PDF-specific ones.
          const messages = [];
          const skippedFonts = Object.keys(allSkipped);
          if (skippedFonts.length) {
            const details = skippedFonts
              .map(font => t('exportSkippedDetail', { font, count: allSkipped[font], plural: allSkipped[font] === 1 ? '' : 's' }))
              .join(t('exportSkippedJoiner'));
            messages.push(t('exportSkippedSummary', { details }));
          }
          if (mismatchCountTotal > 0) {
            messages.push(t('exportMismatchSummary', {
              count: mismatchCountTotal,
              plural: mismatchCountTotal === 1 ? '' : 's',
              addresses: mismatchAddresses.join(', ') + (mismatchCountTotal > mismatchAddresses.length ? '…' : '')
            }));
          }
          if (messages.length) setStatus(messages.join(' '), 'warning');
        }
      } catch (err) {
        console.error(err);
        setStatus('PDF नहीं बन पाया।', 'error');
...
// ---- Final wiring ----
    const pdfSheetPicker = document.getElementById('pdfSheetPicker');
    const pdfSheetCheckboxes = document.getElementById('pdfSheetCheckboxes');
    const pdfPickerConfirmBtn = document.getElementById('pdfPickerConfirmBtn');
    const pdfPickerCancelBtn = document.getElementById('pdfPickerCancelBtn');
    const pdfFontChoiceBox = document.getElementById('pdfFontChoiceBox');
    ...
    exportPdfBtn.addEventListener('click', () => {
      ...
      pdfSheetCheckboxes.innerHTML = currentWorkbook.SheetNames.map((name, i) =>
        '<label style="display:flex; align-items:center; gap:8px;">' +
        '<input type="checkbox" class="pdf-sheet-check" value="' + escapeHTML(name) + '"' +
        (name === currentSheetName ? ' checked' : '') + ' />' +
        escapeHTML(name) + '</label>'
      ).join('');
      // Default: only the sheet currently being viewed is pre-checked —
      // exporting every sheet every time was the old (undesired) default;
      // this makes "just this one" the common case and multi-select opt-in.
      // Font choice only makes sense to show if there's actually a real
      // choice — with no font uploaded, the Unicode guess is the only
      // option anyway, so asking would just be a confusing extra step.
      pdfFontChoiceBox.style.display = activeFontBuffer ? 'block' : 'none';
      pdfSheetPicker.style.display = 'block';
      pdfSheetPicker.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
    });

    pdfPickerCancelBtn.addEventListener('click', () => {
      pdfSheetPicker.style.display = 'none';
    });

    pdfPickerConfirmBtn.addEventListener('click', () => {
      const selected = [...pdfSheetCheckboxes.querySelectorAll('.pdf-sheet-check:checked')].map(cb => cb.value);
      if (!selected.length) {
        setStatus(t('pdfPickerNoneSelected'), 'error');
        return;
      }
      const fontChoice = document.querySelector('input[name="pdfFontChoice"]:checked');
      const useOriginalFont = activeFontBuffer && fontChoice && fontChoice.value === 'original';
      pdfSheetPicker.style.display = 'none';
      exportToPdf(selected, useOriginalFont);
    });
    ...
    if (infoPopupBtn && infoPopupOverlay) {
      infoPopupBtn.addEventListener('click', () => { infoPopupOverlay.style.display = 'flex'; });
      infoPopupCloseBtn.addEventListener('click', () => { infoPopupOverlay.style.display = 'none'; });
      infoPopupOverlay.addEventListener('click', (e) => {
        if (e.target === infoPopupOverlay) infoPopupOverlay.style.display = 'none'; // click outside the panel closes it
      });
    }
    applyTranslations();
