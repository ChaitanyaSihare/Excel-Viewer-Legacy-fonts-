      ...
      for (let c = 0; c < maxCols; c++) {
        ...
        if (forceEnglish) {
          ...
          if (viewMode === 'unicode') {
            ...
            if (needsConversion) {
              const converted = krutiDevToUnicode(cellText, viewerIsProtectedWord);
...
// ---- Resume last session (IndexedDB) ----
    ...
    async function initResumeBanner() {
      ...
      const dateLabel = when.toLocaleDateString(currentLang === 'en' ? 'en-IN' : 'hi-IN', {
        day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit'
      });
      nameEl.textContent = record.fileName + ' — ' + dateLabel;
      ...
      openBtn.addEventListener('click', async () => {
        ...
        if (myLoadToken !== fileLoadToken) return; // user picked a different file while this was loading
        ...
        setStatus('फाइल खोली जा रही है...', 'loading');
        processWorkbookData(new Uint8Array(record.data), record.fileName);
      });
